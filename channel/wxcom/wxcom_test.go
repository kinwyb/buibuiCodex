...
func TestMessageHandlerParsePrivateChatMessage(t *testing.T) {
	handler := NewMessageHandler(nil)

	// 私聊消息：没有 chatid，只有 from.userid
	frame := &WsFrame{
		Cmd: WsCmdCallback,
		Headers: map[string]string{
			"req_id": "test_req_id",
	...
	if msg.UserID != "zhangsan" {
		t.Errorf("Expected UserID 'zhangsan', got '%s'", msg.UserID)
	}

	// 私聊场景 ChatID 应等于 UserID
	if msg.ChatID != "zhangsan" {
		t.Errorf("Expected ChatID 'zhangsan', got '%s'", msg.ChatID)
	}
}
...
func TestMessageHandlerConvertToInbound(t *testing.T) {
	...
	inbound := handler.ConvertToInbound(wsMsg, channel.WxComChannel, "test_bot")

	if inbound.Channel != channel.WxComChannel {
		t.Errorf("Expected Channel %s, got %s", channel.WxComChannel, inbound.Channel)
	}
...
func newTestChannelWithMockWS(t *testing.T, connected bool) *Channel {
	t.Helper()
	handler := NewMessageHandler(nil)
	wsManager := &WsManager{
